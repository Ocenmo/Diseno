...
const MapWithCircle = () => {
  ...
  const [currentTime, setCurrentTime] = useState(null); // Para el slider de ambos
    ...
    if (data && data.length > 0) {
      return data
        ...
        .map((coord) => ({
          ...
          timestamp: new Date(coord.TimeStamp).getTime(), // Guardar timestamp como milisegundos
  ...
  const handleSelectRange = async (startDate, endDate) => {
    ...
    if (data && data.length > 0) {
      ...
      if (filteredCar1.length > 0 && filteredCar2.length > 0) {
        const allTimestamps = [
          ...filteredCar1.map((p) => p.timestamp),
          ...filteredCar2.map((p) => p.timestamp),
        ];
        const min = Math.min(...allTimestamps);
        const max = Math.max(...allTimestamps);
        setMinTime(min);
        setMaxTime(max);
        setCurrentTime(min); // Iniciar en el tiempo mínimo
      } else {
        setMinTime(null);
        setMaxTime(null);
        setCurrentTime(null);
      ...
      setTimestampsCar1(filteredCar1.map((p) => new Date(p.timestamp).toLocaleString("es-CO", { timeZone: "UTC" })));
      setTimestampsCar2(filteredCar2.map((p) => new Date(p.timestamp).toLocaleString("es-CO", { timeZone: "UTC" })));
  ...
  // Función para encontrar el punto más cercano en el tiempo para un vehículo
  const findClosestPoint = (path, time) => {
  ...
  return (
    <div className="flex-1 relative">
      <GoogleMap
        ...
        center={
          selectedCar === "car1" && pathCar1.length > 0
            ...
            : selectedCar === "both" && currentTime
            ? findClosestPoint(pathCar1, currentTime) || findClosestPoint(pathCar2, currentTime)
      ...
      >
        ...
        {(selectedCar === "car1" || selectedCar === "both") && pathCar1.length > 0 && (
          <>
            ...
            {selectedCar === "both" && currentTime && (
              <Marker position={findClosestPoint(pathCar1, currentTime)} />
            )}
          </>
        )}
        ...
        {(selectedCar === "car2" || selectedCar === "both") && pathCar2.length > 0 && (
          <>
            ...
            {selectedCar === "both" && currentTime && (
              <Marker position={findClosestPoint(pathCar2, currentTime)} />
            )}
          </>
        )}
      </GoogleMap>
      ...
      <div className="absolute bottom-5 left-5 z-10 bg-white p-4 border border-gray-300 rounded-xl shadow-md">
        {selectedCar === "car1" && pathCar1.length > 0 && (
          <div>
            ...
            <p>Fecha y hora: {new Date(pathCar1[currentIndexCar1].timestamp).toLocaleString("es-CO", { timeZone: "UTC" })}</p>
          </div>
        )}
        ...
        {selectedCar === "car2" && pathCar2.length > 0 && (
          <div>
            ...
            <p>Fecha y hora: {new Date(pathCar2[currentIndexCar2].timestamp).toLocaleString("es-CO", { timeZone: "UTC" })}</p>
          </div>
        )}
        ...
        {selectedCar === "both" && currentTime && (
          <div>
            ...
            <table className="table-auto">
              ...
              <tbody>
                {["Latitud", "Longitud", "RPM", "Velocidad", "Fecha y hora"].map((label, index) => {
                  const pointCar1 = findClosestPoint(pathCar1, currentTime);
                  const pointCar2 = findClosestPoint(pathCar2, currentTime);
                  return (
                    <tr key={index}>
                      <td>{label}</td>
                      <td className="px-2">
                        {pointCar1
                          ? index < 4
                            ? pointCar1[["lat", "lng", "rpm", "speed"][index]]
                            : new Date(pointCar1.timestamp).toLocaleString("es-CO", { timeZone: "UTC" })
                          : "N/A"}
                      </td>
                      <td className="px-2">
                        {pointCar2
                          ? index < 4
                            ? pointCar2[["lat", "lng", "rpm", "speed"][index]]
                            : new Date(pointCar2.timestamp).toLocaleString("es-CO", { timeZone: "UTC" })
                          : "N/A"}
                      </td>
                    </tr>
                  );
      ...
      {selectedCar === "both" && minTime && maxTime && (
        <div className="absolute w-full max-w-[90%] md:max-w-md h-fit bottom-40 left-1/2 transform -translate-x-1/2 z-10 flex flex-col items-center justify-center border border-black rounded-[99px] bg-[#14213d] text-white shadow-[0_4px_8px_#081c15] px-4 py-3">
          <input
            className="w-full mb-3 accent-yellow-400"
            type="range"
            min={minTime}
            max={maxTime}
            step={1000} // Paso de 1 segundo
            value={currentTime}
            onChange={(e) => setCurrentTime(Number(e.target.value))}
          />
          <p>{new Date(currentTime).toLocaleString("es-CO", { timeZone: "UTC" })}</p>
        </div>
      )}
